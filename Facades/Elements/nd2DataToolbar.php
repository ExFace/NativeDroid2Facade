...
class nd2DataToolbar extends nd2Toolbar
{
    ...
    protected function buildHtmlFloatingActionMenu()
    {
        ...
        if ($widget->countButtons() === 1){
            ...
            $main_button_properties = 'data-mfb-label="' . $btn->getCaption() . '" onclick="' . $this->getFacade()->getElement($btn)->buildJsClickFunctionName() . '()"';
        } else {
      ...
      <li>
        <a href="#" data-mfb-label="{$btn->getCaption()}" class="mfb-component__button--child" onclick="{$this->getFacade()->getElement($btn)->buildJsClickFunctionName()}();">
